---
title: "R21 Sterile Immunity Pseudobulk RNA-seq analysis using DESeq2 GSEA pipeline"
...
```{r apply fgsea}
#read in from file
res_tbl_bound <- read.csv("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/DE_analysis_scrnaseq/results/pseudobulk_DESeq2_lower_level_celltypes_all_DEG_tables.csv")

#revert to named list of DFs using tidytable::group_split (https://github.com/markfairbanks/tidytable/blob/main/R/group_split.R)
res_tbl_list <- res_tbl_bound %>%
  dplyr::select(cell_type, gene, ranking_metric) %>%
  tidytable::group_split(cell_type, .keep = FALSE, .named = TRUE)  %>%
  lapply(., deframe)

devtools::source_url("https://github.com/TranLab/ModuleLists/blob/main/NamedGeneRankList2GseaTable.R?raw=TRUE")
# Github link: https://github.com/TranLab/ModuleLists/blob/main/NamedGeneRankList2GseaTable.R

gsea_res <- sapply(names(res_tbl_list), function(x) NULL)

for(i in names(res_tbl_list)){
  gsea_res[[i]] <- NamedGeneRankList2GseaTable(res_tbl_list[[i]], geneset = c("MSigDB"),
                                          output_directory = "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/DE_analysis_scrnaseq/results",
                                          filename_prefix = "scrnaseq_gsea_no_sampling_l2", fixed_seed = TRUE)
}

#remove NULL elements in list
gsea_res[sapply(gsea_res, is.null)] <- NULL

write_rds(gsea_res, "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/DE_analysis_scrnaseq/results/DESe2_fgsea_MSigDB_res_tabs.rds")
```

## Plot GSEA results as bubble plot

```{r, prepare data to plot gsea results}
gsea_res_bound <- bind_rows(gsea_res, .id = "cell_type")

gsea_res_bound_filtered <- gsea_res_bound %>%
  filter(padj < 0.01) %>%
  dplyr::rename(FDR = padj)

mycomparison <- unique(res_tbl_bound$comparison)
```

```{r plot hallmark, fig.align='center', fig.height=6, fig.width=7}
plot_hm <- gsea_res_bound_filtered %>%
