...
## Separate out CITE-seq data into individual samples

```{r, separate out CITE-seq}
# Apara
...
## Separate out multiome based on "new.ident"
...
Skip this step for DE

```{r equalize cell numbers}
...
set.seed(1234) # check with different seeds
...
```{r, read in then run clustering}
#read in data
# all <- readRDS("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/data/all_15_merged_seurat.rds")
...
```{r integrate data}
all.anchors <- readRDS("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/data/all_anchors.rds")
...
### Load reference dataset and anchor

https://satijalab.org/seurat/articles/multimodal_reference_mapping.html

```{r, load reference and anchor}
#Step 3a: Load the reference dataset (note: this is hard to upload from google Drive dur to its large size)
reference <- LoadH5Seurat("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Transcriptomic reference datasets/pbmc_multimodal.h5seurat")
...
```{r, save remapped all}
saveRDS(refmapped.all, "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/data/refmapped_and_dim_red_no_sampling_all15_minusK613_05172023.rds")
```
...
### Separate out individual cell types

```{r separate cell types}
#Para
...
```{r, split out individual celltypes}
# Predicted celltype L1

#apara
for(i in 1:length(list_apara_l1)){
  Idents(list_apara_l1[[i]]) <- "apara"
  assign(paste0(tolower(gsub(" ", "_", names(list_apara_l1)[i])), "_apar_l1"), list_apara_l1[[i]])
}

#para
for(i in 1:length(list_para_l1)){
  Idents(list_para_l1[[i]]) <- "para"
  assign(paste0(tolower(gsub(" ", "_", names(list_para_l1)[i])), "_par_l1"), list_para_l1[[i]])
}

# predicted celltype L2
#apara
for(i in 1:length(list_apara_l2)){
  Idents(list_apara_l2[[i]]) <- "apara"
  assign(paste0(tolower(gsub(" ", "_", names(list_apara_l2)[i])), "_apar_l2"), list_apara_l2[[i]])
}

#para
for(i in 1:length(list_para_l2)){
  Idents(list_para_l2[[i]]) <- "para"
  assign(paste0(tolower(gsub(" ", "_", names(list_para_l2)[i])), "_par_l2"), list_para_l2[[i]])
}
```

### Merge parasitemic and aparasitemic objects for each celltype

```{r merge and perform DGE}
#source MAST helper function for including random effects
source("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/Kalifabougou Sterile Immunity Manuscript/R21-sc-studies/MAST helper fxs for DGE.R")

# Merge the aparasitemic and parasitemic objects together
# Predicted celltype l1
names(list_para_l1) <- tolower(gsub(" ", "_", names(list_para_l1)))
names(list_apara_l1) <- tolower(gsub(" ", "_", names(list_apara_l1)))
l1_intersect_names <- intersect(names(list_para_l1), names(list_apara_l1))

merged_celltype_deglist_l1 <- merged_celltype_l1 <- sapply(tolower(gsub(" ", "_", l1_intersect_names)),function(x) NULL)
for(i in l1_intersect_names){
   merged_celltype_l1[[i]] <- merge(list_apara_l1[[i]], list_para_l1[[i]], add.cell.ids=c("apara", "para"))
   merged_celltype_l1[[i]] <- PrepSCTFindMarkers(merged_celltype_l1[[i]]) # To make the merged objects comparable, run PrepSCTFindMarkers on each merged element
   merged_celltype_deglist_l1[[i]] <- FindMarkers(merged_celltype_l1[[i]],
                                                  ident.1 = "apara", #note that I use apara as first factor (Prasida uses it as second)
                                                  ident.2 = "para",
                                                  only.pos = F,
                                                  test.use = "MAST",
                                                  assay = "RNA",
                                                  latent.vars = "Sample.ID",
                                                  logfc.threshold = 0.05,
                                                  re.var = "Sample.ID",
                                                  ebayes = F,
                                                  min.pct = 0.1) #DGE using MAST with modification for random fx (see MAST helper script sourced above)
   merged_celltype_deglist_l1[[i]] <- merged_celltype_deglist_l1[[i]] %>%
     rownames_to_column(var = "genesymbol") #move gene symbols from rownames to columns
}
all_degtabs_l1 <- bind_rows(merged_celltype_deglist_l1, .id = "cell_type") %>%
  mutate(annotaton_level = "l1")

# for(i in 1:length(merged_celltype_l1)){
#   assign(names(merged_celltype_l1)[i], merged_celltype_l1[[i]])
# }

# Predicted celltype l2
names(list_para_l2) <- tolower(gsub(" ", "_", names(list_para_l2)))
names(list_apara_l2) <- tolower(gsub(" ", "_", names(list_apara_l2)))
l2_intersect_names <- intersect(names(list_para_l2), names(list_apara_l2))
l2_intersect_names <- l2_intersect_names[l2_intersect_names != "eryth"] #manually remove erythrocytes (too few cells)
merged_celltype_deglist_l2 <- merged_celltype_l2 <- sapply(tolower(gsub(" ", "_", l2_intersect_names)),function(x) NULL)
for(i in l2_intersect_names){
   merged_celltype_l2[[i]] <- merge(list_apara_l2[l2_intersect_names][[i]], list_para_l2[l2_intersect_names][[i]], add.cell.ids=c("apara", "para"))
   merged_celltype_l2[[i]] <- PrepSCTFindMarkers(merged_celltype_l2[[i]]) # To make the merged objects comparable, run PrepSCTFindMarkers on each merged element
   merged_celltype_deglist_l2[[i]] <- FindMarkers(merged_celltype_l2[[i]],
                                                  ident.1 = "apara",
                                                  ident.2 = "para",
                                                  only.pos = F,
                                                  test.use = "MAST",
                                                  ...
                                                  latent.vars = "Sample.ID",
                                                  logfc.threshold = 0.05,
                                                  min.pct = 0.1) #find differential gene expression using MAST
   merged_celltype_deglist_l2[[i]] <- merged_celltype_deglist_l2[[i]] %>%
     rownames_to_column(var = "genesymbol") #move gene symbols from rownames to columns
}
all_degtabs_l2 <- bind_rows(merged_celltype_deglist_l2, .id = "cell_type") %>%
  mutate(annotaton_level = "l2")
```
