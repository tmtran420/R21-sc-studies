...
apara_ADTValidation_plot <-FeaturePlot(all_apara, features = c("CD19.1",
                                        "CD3", 
                                        "CD4.1", 
                                        "CD8a", 
                                        "CD14.1", 
                                        "CD16", 
                                        "CD56-NCAM"),
                reduction = "ref.umap", cols = c("lightgrey", "darkgreen"), min.cutoff = "q45", max.cutoff = "q95", label = T, label.size = 4, 
                repel = TRUE,
                pt.size = 0.005,
                ncol = 4) &
  NoLegend() &
...
apara_RNAValidation_plot <- FeaturePlot(all_apara, features = c("CD19", 
                                        "CD3E", 
                                        "CD4", 
                                        "CD8A", 
                                        "CD14", 
                                        "FCGR3A", 
                                        "NCAM1"),    
                reduction = "ref.umap", min.cutoff = "q45", max.cutoff = "q95", ncol = 4, label = T, label.size = 4, 
             repel = TRUE,
             pt.size = 0.1) &
  NoLegend() &
     theme(axis.text.y = element_text(size=14),
...
para_ADTValidation_plot <- FeaturePlot(all_para, features = c("CD19.1", 
                                                       "CD3", 
                                                       "CD4.1", 
                                                       "CD8a", 
                                                       "CD14.1", 
                                                       "CD16", 
                                                       "CD56-NCAM"),
                reduction = "ref.umap", cols = c("lightgrey", "darkgreen"), min.cutoff = "q45", max.cutoff = "q95", ncol = 4, label = T, label.size = 4, 
             repel = TRUE,
             pt.size = 0.1) &
  NoLegend() &
     theme(axis.text.y = element_text(size=14),
...
apara_ADTValidation_plot <- FeaturePlot(all_para, features = c("CD19", 
                                                               "CD3E", 
                                                               "CD4", 
                                                               "CD8A", 
                                                               "CD14", 
...
```{r plot validation plots}
pdf("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_uMAP_Apara_ADTValidation_05012023.pdf", height=5, width=10)
apara_ADTValidation_plot
dev.off()

pdf("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_uMAP_Para_ADTValidation_05012023.pdf", height=5, width=10)
para_ADTValidation_plot
dev.off()

png("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_uMAP_Apara_ADTValidation_05012023.png", height=5, width=10,
    units = "in", res = 300)
apara_ADTValidation_plot
dev.off()

png("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_uMAP_Para_ADTValidation_05012023.png", height=5, width=10,
        units = "in", res = 300)
para_ADTValidation_plot
dev.off()
```
...
my_cell_freq_plot_lores <- all.cf_citeseq_celltype1 %>%
  ...
  stat_compare_means(label = "p", comparisons = list(c("aparasitemic", "parasitemic")), method = "wilcox.test", symnum.args = symnum.args) +
...
my_cell_freq_plot_hires <-  all.cf_citeseq_celltype2 %>%
  mutate(batch = tolower(batch)) %>%
  ggplot(., aes(x=factor(batch), y=freq, fill = batch, color = batch)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15))) + 
  ylab("percent of total") +
  facet_wrap(~predicted.celltype.l2, scales = "free_y", nrow=4) +
  theme(axis.text.y = element_text(size=14),
        axis.text.x = element_blank(),
        axis.title.y  = element_text(size=14),
        axis.title.x  = element_blank(),
...
```{r save stat plots}
pdf("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_Cell_freq_lowres_l1_stats_04302023.pdf", height=9, width=8)
my_cell_freq_plot_lores
dev.off()

pdf("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_Cell_freq_hires_l1_stats_04302023.pdf", height=8, width=14)
my_cell_freq_plot_hires
dev.off()

png("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_Cell_freq_hires_l1_stats_04302023.png", height=8, width=14, units = "in", res = 300)
my_cell_freq_plot_hires
dev.off()
```

#GSEA bubble plots: Import GSEA results first
```{r Import GSEA results }
...
# Select top significant pathways in each collection and plot seperately
```{r select top significant pathways}
...
signif_plot_pred.celltypel2_Hallmark <- signif_pathways_celltpyel2 %>%
...
signif_plot_pred.celltypel2_C2_KEGG <- signif_pathways_celltpyel2 %>%
  filter(module_type %in% c("MSigDB_C2_kegg_v7.4")) %>%
  mutate(pathway = gsub("KEGG ", "", pathway)) %>%
           ggplot(., aes(x = pathway, y=celltype)) +
           geom_point(aes(size=neglogpadj, color=-NES), alpha = 0.65) +
           scale_size_area(name = expression(-log[10]~adj.~p~value), max_size = bubble_max_size) +
           scale_color_gradient2(low = "blue",
...
```{r save gsea bubble plots, fig.align='center', fig.height=5, fig.width=12}
pdf("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/All15samples_Exvivo_GSEA_FDR5_Hallmark_C2KEGG.pdf", height=4.75, width=11)
ggarrange(signif_plot_pred.celltypel2_Hallmark, signif_plot_pred.celltypel2_C2_KEGG, common.legend = TRUE, nrow=1,
          labels = c("MSigDB_Hallmark_v7.4", "MSigDB_C2_kegg_v7.4"))
dev.off()

```

Create GEX matrices for export for signature scoring
```{r GEX matrices for expor}
#Aara
DefaultAssay(all_apara)<-"RNA"
all_apara_mat<-t(as.matrix(GetAssayData(all_apara)))
write.table(all_apara_mat, "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/Prasida Data/all_exvivo_pools_reclustered_all_apara_refmap_mat.tsv", sep="\t", row.names = TRUE, col.names=NA)
#Para
DefaultAssay(all_para)<-"RNA"
all_para_mat<-t(as.matrix(GetAssayData(all_para)))
write.table(all_para_mat, "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/Prasida Data/all_exvivo_pools_reclustered_all_para_refmap_mat.tsv", sep="\t", row.names = TRUE, col.names=NA)

DefaultAssay(refmapped.all)<-"RNA"
refmapped.all_mat<-t(as.matrix(GetAssayData(refmapped.all)))
write.table(refmapped.all_mat, "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/Prasida Data/all_exvivo_pools_reclustered_refmapped.all_refmap_mat.tsv", sep="\t", row.names = TRUE, col.names=NA)
```

#Import for plot

```{r import data for plot}
setwd("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Presentations/LIG May 2023/Prasida Data/")
apara_scores <- readr::read_tsv("all_exvivo_pools_reclustered_all_apara_refmap_mat.tsv")
apara_scores <- apara_scores %>%
  rename("id" = "...1")

apara_data <- FetchData(all_apara ,c(  "refUMAP_1", "refUMAP_2"))

apara_data <- apara_data %>% 
